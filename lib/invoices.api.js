...
export const getInvoices = async (filters = {}) => {
  ...
  return { data: { invoices: data, pagination: { total: data.length } } };
};
...
export const getMyInvoices = async (filters = {}) => {
  ...
  return {
    data: {
      invoices: data,
      pagination: { total: data.length, pages: Math.ceil(data.length / 10) },
      totalRevenue
    }
  };
};
...
export const getStockTransferInvoices = async (filters = {}) => {
  return { data: { invoices: [], pagination: { total: 0 } } };
};