    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--color-bg)]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-[var(--color-text-muted)]">Loading portal...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex bg-[var(--color-bg)]">
      ...
      >
        <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
          {sidebarOpen ? (
            <path d="M13.5 4.5L4.5 13.5M4.5 4.5l9 9" stroke="var(--color-text-secondary)" strokeWidth="1.5" strokeLinecap="round" />
          ) : (
            <path d="M2.25 4.5h13.5M2.25 9h13.5M2.25 13.5h13.5" stroke="var(--color-text-secondary)" strokeWidth="1.5" strokeLinecap="round" />
          )}
        </svg>
      </button>
        ...
        <div className="px-5 py-4 border-b border-[var(--color-border)]">
          <Link href="/portal/dashboard" className="flex items-center gap-3 group">
            ...
            <div>
              ...
              <p className="text-[10px] text-[var(--color-text-muted)] uppercase tracking-widest font-medium">Distributor</p>
            </div>
          </Link>
        </div>
        ...
        <div className="px-4 py-4 border-t border-[var(--color-border)]">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-[var(--color-bg)] mb-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-[var(--color-text-secondary)] truncate">{user?.business_name || user?.name}</p>
              <p className="text-xs text-[var(--color-text-muted)] capitalize">Distributor</p>
            </div>
          </div>
          <button
            onClick={logout}
